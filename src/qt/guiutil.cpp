...
{
    // Convert everest:// to everest:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("everest://"))
    {
        uri.replace(0, 12, "everest:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Everest.lnk";
}
...
{
    return GetAutostartDir() / "everest.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Everest-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  everest-qt [" + tr("command-line options") + "]                     " + "\n";
