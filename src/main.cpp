...
#if defined(NDEBUG)
# error "Everest cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 6 * 60 * 30; // 3 hours
...
const string strMessageMagic = "Everest Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    }else if(pindexBest->nHeight <= 1100){
	nSubsidy = 30 * COIN;
     }
  else {
        nSubsidy = 0 * COIN;
...
if(pindexBest->nHeight <= (14400)) {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1/50; //10% for 10 days to start
	}
	else if(pindexBest->nHeight <= (36000)) {
       nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 10; //5000% for 15 days
       }
	else if(pindexBest->nHeight <= (57600)) {
       nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 8; //4000% for 15 days
       } 
    else if(pindexBest->nHeight <= (93600)) {
       nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 6; //3000% for 25 days 
       } 
	else if(pindexBest->nHeight <= (165600)) {
       nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 4; //2000% for 50 days
      } 
	else if(pindexBest->nHeight <= (309600)) {
       nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2; //1000% for 100 days
      } 
	else {
	if(pindexBest->nHeight % 100 == 0){
       nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 4; //2000% Jackpot 100
	}else if(pindexBest->nHeight % 25 == 0){
	nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2; //1000% Jackpot 25,50,75
	}else if(pindexBest->nHeight % 10 == 0){
	nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1; //500% Jackpot 10,20,30,40,60,70,80,90
	}else{
	nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1/2; //250% No Jackpot
	}
      } 
